...
import { MOVEMENTS, ROUTINES, UNIT, GEAR, rung, describeSet, gearFor } from '../data/exercises.js';
import {
  ...
  render() {
    ...
    return h`
      <div class="screen">
        ...
        <div class="stack">
          ${raw(roundCard(done, goal, today, stam))}
          ...
          <p class="muted center" style="font-size:.78rem;margin:-4px 0 0">
            Or tap the number beside one as you finish it. Under the target still counts — open a row to type what you actually did.
          </p>
...
function roundCard(done, goal, today, stam) {
  const pct = goal ? Math.min(1, done / goal) : 0;
  const line = done === 0
    ? 'Nothing logged yet today. One set is a start.'
    : done >= goal
      ? (done > goal ? `${done} rounds — past what you set out to do.` : 'Round done. Anything more is a bonus.')
    ...
    <div class="card trhead">
      <div class="row-between">
        <div>
          ...
          <div class="trhead__l">round${goal === 1 ? '' : 's'} today</div>
        </div>
...
function doLog(mid, reps, el) {
  ...
  if (res.roundFinished) {
    ...
    confetti({ count: 70 });
    toast(`Round ${res.rounds} done`, { icon: icon('jasad'), tone: 'good', ms: 2600 });
  } else if (res.capped) {
...
function doLogRound(el) {
  ...
  toast(`Round ${rounds} done`, { icon: icon('jasad'), tone: 'good', ms: 2600 });
  refresh();
}
...
function openEditor() {
  ...
  sheet({
    ...
    body: h`
      <div class="stack">
        <label class="field">
          <span>How many rounds are you aiming for?</span>
          <div class="chiprow" id="tr-goal">
            ${[1, 2, 3, 5].map((n) => raw(h`<button class="chip${n === goal ? ' is-on' : ''}" data-goal="${n}">${n}</button>`))}
          </div>
          <span class="hint">A target to aim at, never a requirement. Half a round is still logged, still scored, and still shows up on the graph.</span>
        </label>
      ...
      <span class="row-between">
        <span class="grow">
          ...
          <span class="trtoday__s">${sets === 0
            ? `${goal} round${goal === 1 ? '' : 's'} today · nothing logged yet`
            : `${done} of ${goal} round${goal === 1 ? '' : 's'} · ${dayScore(state, key)} points`}</span>
        </span>
        <span class="listrow__chev">›</span>
      </span>
    </button>`;
